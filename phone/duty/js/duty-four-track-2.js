...
function getTrackFromBaidu(userCode,userName,startTime){
    ...
    $.getJSON(param+"&page_index="+pageIndex+"&start_time="+startTime+"&end_time="+endTime+"&is_processed=1",function(info){
        ...
        //向上取整
        count=Math.ceil(info.total/5000);
    ...
    for(var i=1;i<=count;i++){
        $.getJSON(param+"&page_index="+count+"&start_time="+startTime+"&end_time="+endTime+"&is_processed=1",function(info){
            //将每次请求到的数据进行合并
...
function beginDrawMap(allDataArray){
    ...
    uexBaiduMap.open(0, t, w, h-t, CENTER_POINT.longitude, CENTER_POINT.latitude, function(){
           ...
           //判断是否设备平台，如果是android的话进行分段画，ios不执行这个操作
           // if(platForm == "1"){
               // //开始画线，由于数据不可能超过10000个，所以直接调用即可
               // var pointSpliceDate=sliceArray(allDataArray,15);
               // for(var i=0;i<pointSpliceDate.length;i++){
                   // if(pointSpliceDate[i].length == 1 && i == pointSpliceDate.length-1){
                       // pointSpliceDate[i-1].concat(pointSpliceDate[i]);
                       // pointSpliceDate.pop();
                   // }
               // }
               // for(var i=0;i<pointSpliceDate.length;i++){
                   // drawsOverlayMap(pointSpliceDate[i],1);
               // }
           // }else{
...
function handlePoints(allDataArray){
      var arrIndex=new Array();
        for(var i=0;i<allDataArray.length;i++){        
         ...
         var MAX_POINT_LENGTH=0;
         //稀释因子，表示最大能在地图上划线的个数是10000个，按照一万来稀释
         MAX_POINT_LENGTH=parseInt(Math.ceil(allDataArray.length/10000));
         if(allDataArray.length >= 10000){
             var pointDate=new Array();
             for(var i=1;i<allDataArray.length;i=i+MAX_POINT_LENGTH){
                 pointDate.push(allDataArray[i]);
             }
             allDataArray=pointDate;
         } 
