...
var vm = new Vue({
    ...
    methods: {
        ...
        submitDataFn: function() {
            ...
            submitData['personQuestionID'] = self.obj.id;
            submitData['content'] = self.obj.content;
            submitData['dutyDate'] = appcan.locStorage.getVal('dutyDeptTime');
            
            appcan.window.publish('duty-four-2-question-shade', 'show');
            addConfirm({
                content: '确定提交吗？',
                yes: function(i){
                    sendData(submitData);
                ...
                no: function (i) {
                    appcan.window.publish('duty-four-2-question-shade', 'hide');
                    layerRemove(i);
                }
            });
        },
        saveDataFn:function(){
            ...
            if (vm.obj.status == 'todo' && vm.isSubmit == 'false') {
             ...
             if (isNeedDraft == true) {
                 ...
                 saveDraft();
                 appcan.window.publish('duty-four-2-question-shade', 'hide');
                 appcan.window.publish('duty-four-2-question-close');
            }else {
                // 关闭页面
                appcan.window.publish('duty-four-2-question-shade', 'hide');
                appcan.window.publish('duty-four-2-question-close');
...
(function($) {
    appcan.ready(function() {
        ...
        var parentData= JSON.parse(appcan.locStorage.getVal("duty-four-2-question"));
        if (isDefine(parentData)) {
            // 基础信息
            vm.obj.id = parentData.id;
            vm.obj.status = parentData.status;
            vm.obj.person = parentData.person;
            vm.obj.question = parentData.question;
            var questionData = parentData.mustPersonQuestionDate;
            if (isDefine(questionData)) {
                // 提交后的信息 
                vm.obj.content = questionData.content;
...
function sendData(params){
    ...
    ajaxRequest(json, function(data, e){
        appcan.window.publish('duty-four-2-question-shade', 'hide');
        if(e == 'success'){
            appcan.window.publish('duty-four-2-question');
            appcan.window.publish('duty-four-2-question-close');
        }else{
