...
(function($) {
    ...
    //左滑显示操作按钮
    $(document).on('swipeleft', '.myli li', function(e){
        e.stopPropagation();
        var that = $(this);
    ...
    //右滑该项, 列表恢复原位
    $(document).on('swiperight', '.myli li', function(e){
        var that = $(this);
    ...
    //点击按钮本身, 列表不恢复原位
    $('.myli').on('tap', '.myli-btn', function(e){
        e.stopPropagation();
    });
...
function showPositionOrTrack(flag,data){
    ...
    //该单位值班账号，可能为空
    var dutyAccount=data.dutyPersons.dutyCode;
