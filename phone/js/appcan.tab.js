appcan.define("tab", function($, exports, module) {
    ...
    TabView.prototype = {
        ...
        badge : function(index, count) {
            /**
             * 修改了当count 为0时没有动态删除元素，和当刚开始为0时没有动态加上元素。 
             */
             if(count>0){
                 if($("#badge_" + index, self.ele).length>0){
                     if(count>99){
                         $("#badge_" + index, self.ele).html("99+");
                     }else{
                         $("#badge_" + index, self.ele).html(count);
                     }
                 }else{
                     if(count>99){
                          $("div[data-index='"+index+"']").append("<div class='ulev bc-text-head sc-bg-alert tab_badge' id='badge_"+index+"'>"+"99+"+"</div>");
                     }else{
                          $("div[data-index='"+index+"']").append("<div class='ulev bc-text-head sc-bg-alert tab_badge' id='badge_"+index+"'>"+count+"</div>");
                     }
                     
                 } 
             }else{
                     $("#badge_" + index, self.ele).remove();
             }
