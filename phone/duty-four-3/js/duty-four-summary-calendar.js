...
function getAjaxData(Y, M, D) {
    ...
    appcan.locStorage.setVal('summary-dutyDay-stamp', (Date.parse(new Date(Y, M, D)) / 1000));
    appcan.locStorage.setVal('duty-four-summary-dutyDay', timeStemp(Date.parse(new Date(Y, M, D)) / 1000, 'yyyy-MM-dd').date);
    loadDutyFourDate(Date.parse(new Date(Y, M, D)) / 1000);
};
