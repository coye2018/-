            ...
            if (vm.tagIndex == 0) {
                ...
                if (!isDefine(this.searchIpt)) {
                    ...
                    for (var i = 0; i < vm.listmiddleDataQuery.length; i++) {
                        ...
                        if(listmiddle.content.indexOf(this.searchIpt) > -1 || listmiddle.dodepartname.indexOf(this.searchIpt) > -1) {
                ...
                if(!isDefine(this.searchIpt)){
                    ...
                    for (var i = 0; i < vm.listrightDataQuery.length; i++) {
                        ...
                        if (listright.content.indexOf(this.searchIpt) > -1 || listright.dodepartname.indexOf(this.searchIpt) > -1) {
...
function handleDutyProblem(obj, idx) {
    ...
    for (var b = 0; b < obj.length; b++) {
        ...
        var receivetime_format = (isDefine(th.receivetime) ? timeStemp(th.receivetime, 'MM-dd HH:mm').commonDate : '--');
        ...
        var temp = {
            ...
            sendtime: th.sendtime, //转派时间
            sendtime_format: sendtime_format, //转派时间(格式化)
            receivetime: th.receivetime, //接收时间
            receivetime_format: receivetime_format, //接收时间(格式化)
            finishtime: th.finishtime, //完成时间(包括验收完成/已关闭/无法整改时间)
            finishtime_format: finishtime_format, //完成时间(格式化)
            state: state, //状态
            stateString: dutyDisCommon[state].text, //状态对应的文本字段
            colour: dutyDisCommon[state].colour, //状态对应的颜色字段
            dodepartname: th.dodepartname, //责任单位
            managerdepartname: th.managerdepartname,//转派单位
            senddepartname: th.senddepartname//报送单位
        };
...
function loadDutyProblem(page) {
    ...
    ajaxRequest(json, function(data, e) {
        if (e == 'success') {
            ...
            console.log(JSON.stringify(vm.listmiddleData));
        } else {
            vm.nonetRecive = true;
...
(function($) {
    ...
    appcan.ready(function() {
        //4待下发5待验收6已关闭, js方便起见就都减4, 调接口才加回来
        ...
        //重新加载列表数据
        appcan.window.subscribe('duty-problem-dist-reload', function(e) {
