...
var vm = new Vue({
    ...
    methods: {
        ...
        submitDataFn: function() {
            ...
            appcan.window.publish('duty-four-3-place-shade', 'show');
            addConfirm({
                content: '确定提交吗？',
                yes: function(i){
                    sendData(submitData);
                ...
                no: function (i) {
                    appcan.window.publish('duty-four-3-place-shade', 'hide');
                    layerRemove(i);
                }
            });
        },
        saveDataFn:function(){
            ...
            if (isNeedDraft == true) {
                 ...
                 saveDraft();
                 appcan.window.publish('duty-four-3-place-shade', 'hide');
                 appcan.window.publish('duty-four-3-place-close');
                 appcan.window.publish('duty-four-3-place-draft');
...
(function($) {
    appcan.ready(function() {
        ...
        var parentData= JSON.parse(appcan.locStorage.getVal("duty-four-3-place"));
        // 基础信息
        if (isDefine(parentData)) {
            vm.obj.id = parentData.id;
            vm.obj.status = parentData.status;
            vm.obj.startTime = parentData.task_start_time;
            vm.obj.endTime = parentData.task_end_time;
            vm.obj.place = parentData.task_area;
...
function sendData(params){
    ...
    ajaxRequest(json, function(data, e){
        appcan.window.publish('duty-four-3-place-shade', 'hide');
        if(e == 'success'){
            appcan.window.publish('duty-four-3-place');
            appcan.window.publish('duty-four-3-place-close');
        }else{
