(function($) {
    ...
    appcan.ready(function() {
        ...
        var ipbox = $('#login_input'),
            ipboxW = parseInt($('.login-body').width()),
            ipboxH = parseInt(ipbox.height()),
            ipboxT = parseInt(ipbox.offset().top);
        
        appcan.window.openPopover({
            name: 'login-input',
            url: 'login-input.html',
            left: 0,
            top: ipboxT,
            width: ipboxW,
            height: ipboxH
        ...
        appcan.window.subscribe('login-input-json', function(msg){
            var ujson = JSON.parse(msg);
            
            if(!isDefine(ujson.uname)){
                layerToast('请输入账号名。');
                return;
            }else if(!isDefine(ujson.upswd)){
                layerToast('请输入密码。');
                return;
            }
            login(ujson.uname, ujson.upswd);
        });
    ...
    $('#loginBtn').on('click', function(){
        appcan.window.publish('login-input', '1');
    ...
    function login(uname,upswd){
        ...
        layer.open({
            ...
            ,success: function(elem){
                ...
                appcan.window.publish('login-opacity', '1');
            }
        });
        //这个是获取app的信息，其中有版本号
            ...
            success: function(data) {
                ...
                if(loginJson.success){
                    ...
                    //  if(data.result=="1"){
                        ...
                        appcan.window.publish('login-opacity', '0');
                        //加载群头像
