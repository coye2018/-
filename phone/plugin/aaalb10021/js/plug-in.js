    ...
    var plugInUploaderMgr = {
        upload: function (json, cb) {
            ...
            for (var i = 0; i < json.filePath.length; i++) {
                var param = {
                    "url": json.serverUrl + "&token=" + appcan.locStorage.getVal("token"),
                    "type": 2
                }
                ...
                if (json.hasOwnProperty("sessionId")) {
                    var headJson = {
                        "Cookie": json.sessionId
                    };
                    uexUploaderMgr.setHeaders(uploader, JSON.stringify(headJson));
                }

                uexUploaderMgr.uploadFile(uploader, json.filePath[i], "upload", json.quality, 640, function (
...
window.appcan && appcan.define('plugInDownloaderMgr', function ($, exports, module) {
    var plugInDownloaderMgr = {
        download: function (json, cb) {
            ...
            uexDownloaderMgr.download(downloader, json.serverUrl, json.downloadUrl, 1, function (fileSize, percent,
                status) {
                switch (status) {
                case 0:
                    var returnJson = {
                    ...
                    if (platForm == "1") {
                        ...
                        appcan.request.ajax({
                            ...
                            success: function (json2) {
                                ...
                                if (data.success) {
                                    var isConfirmUpdate = appcan.locStorage.getVal("isConfirmUpdate");
                                    if (isConfirmUpdate == "true") {
                                        appcan.locStorage.setVal("isConfirmUpdate", "false");
                                        uexWindow.confirm({
                                            title: "",
