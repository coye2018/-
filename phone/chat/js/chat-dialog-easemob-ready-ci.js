...
// //监听index.js存的图片信息
// appcan.window.subscribe("photoLibraryObserver",function(msg){
         // photoPath = JSON.parse(msg);
// //判断chat-box的高度,赋值手机屏幕的高度
    // //var phoneHeight =parseInt($(window).height());
    // //alert($(window).height()); 
    // if(isScollScreen){
        // //$("#chat-box").height($("#chat-box").height()-phoneHeight);
        // isScollScreen=false;
    // } 
    // if(parseInt($("#chat-box").height())<phoneHeight){
        // //$("#chat-box").height(phoneHeight);
        // isScollScreen=true;
    // }
//           
        // var keyHgh=uexChatKeyboard.getInputBarHeight();
        // if(isDefine(photoPath)){
                // $(".choosePic").css("display","block");
                // var windowHeight=$(window).height();
                // //alert(phoneHeight+"  "+windowHeight);
                // if(flagBommom){
                    // $(".choosePic").css("bottom",keyHgh+phoneHeight-windowHeight);
                // }else{
                    // $(".choosePic").css("bottom",keyHgh);
                // }
//                 
...
function onuexChatKey(){
    ...
    uexChatKeyboard.onKeyBoardShow=function(json){
        ...
        if(status=="1"){
            ...
            // photo=appcan.locStorage.getVal("photo");
            ...
            if(platForm=="1"){
                ...
                uexChatKeyboard.changeWebViewFrame($('#chat-box').height());
                 ...
                 // if(isDefine(photo)){
                    ...
                    // if(isDefine(photo.path)){
                        // var date1=new Date().getTime();
                        // if(parseInt(photo.date)-parseInt(photo.date1)>5000){
                            // appcan.locStorage.remove("photo");
                        // }
//                         
                        // var keyHgh1=uexChatKeyboard.getInputBarHeight();
                        // $(".choosePic").css("display","block");
                        // var windowHeight=$(window).height();
                        // //alert(phoneHeight+"  "+windowHeight);
                        // $(".choosePic").css("bottom",keyHgh1+phoneHeight-windowHeight);
                        // $(".picbox img").attr("src",photo.path);
                        // timerSetwo=setTimeout(function(){
                               // $(".choosePic").css("display","none");
                               // appcan.locStorage.remove("photo");
                               // clearTimeout(timerSetwo);
                               // },10000);
                    // }else{
                        // $(".choosePic").css("display","none");
                    // }   
            ...
            //if(parseInt($("#chat-box").height())<0){
            //    $("#chat-box").height(phoneHeight);
            //}else if(parseInt($("#chat-box").height())>parseInt(phoneHeight)){
            //    $("#chat-box").height(phoneHeight);
            //}
            // if(isScollScreen){
                // //$("#chat-box").height($("#chat-box").height()-phoneHeight);
            ...
            // var keyHgh2=uexChatKeyboard.getInputBarHeight();
            // $(".choosePic").css("bottom",keyHgh2);
            var platForm = appcan.locStorage.getVal("platForm");
            if(platForm=="1"){
                setBottomHeight();
                var keyHeight=uexChatKeyboard.getInputBarHeight();
...
function sendPic(data){
    ...
    var para = null;
    var check = function() {
        var imgwid = imgele.width,
            imghig = imgele.height;
        if(imgwid > 0 || imghig > 0) {
            EXIF.getData(imgele, function() {
                ...
                showMy("image", data, MyMessageId, para);
                sendPicData(MyMessageId, data, orientation);
            });
        }
    };
    var set = setInterval(check, 1000);
}

function sendPicData(MyMessageId, data, ori){
    var param = {
        "username":groupChatId,//单聊时聊天人的userid或者群聊时groupid
        "chatType":peopleChatType,//0-单聊,1-群聊
        "filePath":data,//图片文件路径
        "ext":appcan.locStorage.getVal("userName")+"#"+MyMessageId+"#"+ori,
        "forceNotification":true
        //"pushTitle":appTitle
    };
...
function openVideo(item){
    ...
    appcan.locStorage.setVal("videoName",item.messageBody.displayName);
    //Android
    if(platForm=="1"){
        appcan.window.open({
            name: 'chat-video',
...
function getHistoryMessageById(){
    var hisMessageId;
    if(vm.Chat.length==0){
         hisMessageId="";
    }else{
