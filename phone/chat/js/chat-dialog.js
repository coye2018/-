...
Vue.use(VueLazyload, {
    ...
    //error: 'http://10.10.236.215:8008/jeezz-single//upload/8a8a8bfc59977acf0159978d29db0002/personHead/crop_temp_1503286224167.jpg'
});
    ...
    // 上下拉加载
     mescroll_one = new MeScroll('mescroll-1', {
        down: {
            ...
            callback: getHistoryMessageById
        },
        up: {
            use:false
        }
...
//          uexAlbumLsn.stopListen(); 
    ...
    window.uexOnload =  function() {
        ...
        function() { //After Release or call reload function,we reset the bounce
            ...
            flag1=false;
            if(!isTextBig){
                //vm.Chat=s.concat(vm.Chat);
                 getHistoryMessageById(); 
            }
            scrollbox.reset();
        }).on("onReloading",
        function(a) {
            console.log(1); //如果onReloading状态，拖动将触发该事件
        }).on("dragToReload",
        function() {
            console.log(2);
        }).on("draging",
...
$('#test').one('click', function(){
    var that = $(this), hid = 'hide';
    that.find('.chat-docu-data').addClass(hid);
    
    $('.prog-chat').removeClass(hid).prog({
        total: 100,
        normal: 0,
        hastext: false
    });
    ...
    var ttt = setInterval(function(){
        tt++;
        if(tt>=100){
            $('.prog-chat').prog('destroy');
            $('.chat-docu-data').removeClass(hid);
            that.find('.chat-docu-status span').text('已下载')
            clearInterval(ttt);
        }
        $('.prog-chat').prog('set', {
            normal: tt
